...
const project_tree_1 = require("./project_tree");
...
connection.onInitialize((params) => {
    const result = { capabilities: { textDocumentSync: node_1.TextDocumentSyncKind.Incremental, completionProvider: { resolveProvider: true } } };
...
function validateBasicTree() {
    connection.workspace.getWorkspaceFolders().then((v) => {
        if (!v)
            ...
            (0, project_tree_1.validateProjectTree)(connection, v);
    });
}
connection.onInitialized(() => {
    validateBasicTree();
